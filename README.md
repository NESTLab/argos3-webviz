...
You can [Download pre-compiled binaries](releases)
...
### Web Client
The web client code is placed in `client` directory (or download it as zip from the [releases](releases)). This folder needs to be *served* through an http server(for example `apache`, `nginx`, `lighthttpd`).
