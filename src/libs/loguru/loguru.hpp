...
#undef MAX
  ...
  /* Everything with a verbosity equal or greater than g_stderr_verbosity will
  be written to stderr. You can set this in code or via the -v argument. Set to
  loguru::Verbosity_OFF to write nothing to stderr. Default is 0, i.e. only log
  ERROR, WARNING and INFO are written to stderr.
  */
  ...
  */
  ...
  /*  Should be called from the main thread.
          ...
          loguru::init() will look for arguments meant for loguru and remove
     them. Arguments meant for loguru are: -v n   Set loguru::g_stderr_verbosity
     level. Examples: -v 3        Show verbosity level 3 and lower. -v 0 Only
     show INFO, WARNING, ERROR, FATAL (default). -v INFO     Only show INFO,
     WARNING, ERROR, FATAL (default). -v WARNING  Only show WARNING, ERROR,
     FATAL. -v ERROR    Only show ERROR, FATAL. -v FATAL    Only show FATAL. -v
     OFF      Turn off logging to stderr.
     ...
     the -v flag. Note that -v does not affect file logging (see
     loguru::add_file).
  ...
  */
  ...
  /*  Get the verbosity enum value from a custom 4-character level name, if one
     exists. If the name does not match a custom level name, Verbosity_INVALID
     is returned.
  */
  LOGURU_EXPORT
...
#if LOGURU_USE_FMTLIB
  LOGURU_EXPORT
  LOGURU_NORETURN void vlog_and_abort(
  ...
  // If g_flush_interval_ms is set to non-zero, this will be called
  // automatically this often. If not set, you do not need to call this at all.
  LOGURU_EXPORT
  void flush();
  ...
  /* Returns the thread name for this thread.
     On OSX this will return the system thread name (settable from both within
     and without Loguru). On other systems it will return whatever you set in
     ...
     is a good number for length. right_align_hext_id means any hexadecimal
     thread id will be written to the end of buffer.
  */
  LOGURU_EXPORT
  void get_thread_name(
    char* buffer, unsigned long long length, bool right_align_hext_id);
  ...
  */
  ...
  // Example: demangle(typeid(std::ofstream).name()) ->
  ...
  */
  ...
  // template<typename Printer>
  // EcEntryLambda<Printer> make_ec_entry_lambda(const char* file, unsigned
  // line, const char* descr, Printer&& printer)
  // {
...
#define ERROR_CONTEXT(descr, data)                                      \
  ...
  /*
          #define ERROR_CONTEXT(descr, data)                                 \
                  const auto LOGURU_ANONYMOUS_VARIABLE(error_context_scope_)( \
                          loguru::make_ec_entry_lambda(__FILE__, __LINE__,
     descr,    \
  ...
  */
  ...
  /*
          ...
          The handle is valid as long as the current thread has no changes to
     its error context stack. You can pass the handle to
     loguru::get_error_context on another thread. This can be very useful for
     when you have a parent thread spawning several working threads, and you
     want the error context of the parent thread to get printed (too) when there
     is an error on the child thread. You can accomplish this thusly:
  ...
  */
  ...
  /*
  You can add ERROR_CONTEXT support for your own types by overloading
  ec_to_text. Here's how:
  ...
  Any file that include some.hpp:
          void foo(MySmallType small, const MyBigType& big)
          {
                  ...
                  ERROR_CONTEXT("Big",   &big);  // `big` should not change
  during this scope!
                  ....
          }
  */
...
#undef DEFINE_CHECK_OP_IMPL
  ...
  /*  GLOG trick: Function is overloaded for integral types to allow static
     const integrals
          declared in classes and not defined to be used as arguments to CHECK*
     macros. */
  template <class T>
