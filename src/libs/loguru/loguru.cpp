...
#define localtime_r(a, b) \
  localtime_s(            \
    b,                    \
    a)  // No localtime_r with MSVC, but arguments are swapped for localtime_s
#else
#include <signal.h>
...
#define VTSEQ(ID) ("\x1b[" #ID "m")
  ...
  void file_log(void* user_data, const Message& message) {
    ...
    if (
      duration_cast<seconds>(steady_clock::now() - file_abs->last_check_time) >
      check_interval) {
  ...
  void file_reopen(void* user_data) {
    ...
    int ret;
    if (
      !file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 ||
  ...
  static void parse_args(int& argc, char* argv[], const char* verbosity_flag) {
    ...
    for (int arg_it = 1; arg_it < argc; ++arg_it) {
      ...
      if (
        strncmp(cmd, verbosity_flag, arg_len) == 0 &&
        ...
        if (req_verbosity != Verbosity_INVALID) {
          ...
          CHECK_F(
            ...
            "Invalid verbosity. Expected integer, INFO, WARNING, ERROR or OFF, "
            "got '" LOGURU_FMT(s) "'",
            value_str);
        }
      } else {
...
#define getcwd _getcwd
  ...
  bool remove_callback(const char* id) {
    std::lock_guard<std::recursive_mutex> lock(s_mutex);
    auto it = std::find_if(
      begin(s_callbacks), end(s_callbacks), [&](const Callback& c) {
        return c.id == id;
      });
  ...
  strncpy_s(
    get_thread_name_win32(), LOGURU_THREADNAME_WIDTH + 1, name, _TRUNCATE);
...
namespace loguru {
  ...
  void install_signal_handlers(bool unsafe_signal_handler) {
    ...
    for (const auto& s : ALL_SIGNALS) {
      CHECK_F(
        sigaction(s.number, &sig_action, NULL) != -1,
        "Failed to install handler for " LOGURU_FMT(s) "",
        s.name);
    }
  }
}  // namespace loguru
